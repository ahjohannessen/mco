...
class FolderSourceSpec extends UnitSpec {
  ...
  val run = StubRun(stub)

  "FolderSource companion" should "create source given directory" in {
    val io = FolderSource("storage", SimpleClassifier)
    run.value(io) should not be empty
  }

  it should "not create source given a file" in {
    val io = FolderSource("another file", SimpleClassifier)
    run.value(io) shouldBe empty
  }

  "FolderSource#list" should "list objects created using provided media & classifier" in {
    val Some(src1) = run.value(FolderSource("storage", SimpleClassifier, ArchiveMedia))
    val Some(expectedMedia1) = run.value(ArchiveMedia("storage/archive.zip"))
    run.value(src1.list).loneElement._1 shouldEqual run.value(SimpleClassifier(expectedMedia1))
    val Some(src2) = run.value(FolderSource("storage", SimpleClassifier, FolderMedia))
    val Some(expectedMedia2) = run.value(FolderMedia("storage/folder"))
    run.value(src2.list).loneElement._1 shouldEqual run.value(SimpleClassifier(expectedMedia2))
  }

  "FolderSource#add" should "pull file or folder from filesystem & source" in {
    val Some(src) = run.value(FolderSource("storage", SimpleClassifier, FolderMedia))
  ...
  "FolderSource#remove" should "remove file or folder from filesystem & source" in {
    val Some(src) = run.value(FolderSource("storage", SimpleClassifier, ArchiveMedia, FolderMedia))
  ...
  "FolderSource#rename" should "rename file or folder on disk" in {
    ...
    folderContents should not contain key ("folder")
  ...
  // TODO - separate media and classifiers existing
}
